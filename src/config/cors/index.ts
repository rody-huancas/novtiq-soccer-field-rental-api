 ...
 **/
const origin = (url: string, callback: (m: Error | null, b: boolean) => void) => {
  if (!url) return callback(null, true) // La url no existe

  // Verificar si la url está dentro de los dominios permitidos
  const isAllowedDomain = (allowedDomains as string[]).some((allowedDomain) => url.includes(allowedDomain))

  if (isAllowedDomain) return callback(null, true)

  const msg = new Error(`Este sitio '${url}' no tiene acceso. Solo los dominios específicos pueden acceder a él.`)
